...
import { selectShipLength, setDataOnBoard1, setDataOnBoard2 } from "./store/boardSlice.ts";
...
connection.on("RefreshBoards", args => {
  GetBoard();
})
...
export function SetBoard(){
  ...
  connection.invoke(`SetBoard`, {BoardId: currentState.board.board1.boardId, ShipSize: currentState.board.board1.shipLength, X:currentState.board.board1.hoverX, Y:currentState.board.board1.hoverY})
...
export async function GetRivalBoard(){
  const currentState = store.getState();
  await connection.invoke("SendRivalBoard", currentState.board.board1.rivalBoardId)
}
...
export async function HitBoard(){
  const currentState = store.getState();
  await connection.invoke("HitBoard", currentState.board.board2.boardId, currentState.board.board2.hoverX, currentState.board.board2.hoverY)
}
